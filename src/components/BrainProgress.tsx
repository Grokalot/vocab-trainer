...
const VIEW_SIZE = 612.001;
const HEAD_IMAGE = '/brain-and-head.svg';

/** Fallback until the path is measured in the DOM. */
const DEFAULT_BRAIN_BOUNDS = { minY: 58, maxY: 399 };

/** Inner brain subpath from brain-and-head.svg (same viewBox). */
const BRAIN_PATH =
  'M461.327,209.858c-6.039,7.651-12.918,9.146-16.556,9.346' +
...
export default function BrainProgress({ learned, total }: BrainProgressProps) {
  const riseClipId = `brain-rise-${useId().replace(/:/g, '')}`;
  const measureRef = useRef<SVGPathElement>(null);
  const [brainBounds, setBrainBounds] = useState(DEFAULT_BRAIN_BOUNDS);

  useLayoutEffect(() => {
    const box = measureRef.current?.getBBox();
    if (!box || box.height <= 0) return;
    setBrainBounds({ minY: box.y, maxY: box.y + box.height });
  }, []);

  const brainHeight = brainBounds.maxY - brainBounds.minY;
  const fillPercent = useMemo(
    () => (total > 0 ? Math.min(1, learned / total) : 0),
    [learned, total],
  );
  const filledHeight =
    learned > 0 ? Math.max(brainHeight * fillPercent, 6) : 0;
  const fillY = brainBounds.maxY - filledHeight;
  const percentLabel = total > 0 ? Math.round(fillPercent * 100) : 0;

  return (
    <div className="brain-progress">
      <div
        className="brain-progress-art"
        role="img"
        aria-label={`${learned} of ${total} words learned`}
      >
        <img src={HEAD_IMAGE} className="brain-ghost" alt="" aria-hidden />
        <svg
          className="brain-fill-overlay"
          viewBox={`0 0 ${VIEW_SIZE} ${VIEW_SIZE}`}
          preserveAspectRatio="xMidYMid meet"
          aria-hidden
        >
          <defs>
            <clipPath id={riseClipId}>
              <rect x="0" y={fillY} width={VIEW_SIZE} height={filledHeight + 2} />
            </clipPath>
          </defs>
          <path ref={measureRef} d={BRAIN_PATH} fill="none" visibility="hidden" />
          <path
            className="brain-fill-path"
            d={BRAIN_PATH}
            fill="#ff6b9d"
            clipPath={`url(#${riseClipId})`}
          />
        </svg>
      </div>

      <p className="brain-progress-label">
        {learned} / {total} learned · {percentLabel}%
      </p>
    </div>
  );
}
